...
capc_laea<-spTransform(capc, CRS("+proj=laea"))
wmap_laea<- spTransform(wmap, CRS("+proj=laea"))    
countries_laea<-spTransform(countries, CRS("+proj=laea"))

capc_laea_df<-data.frame(capc_laea)
wmap_laea_df<-fortify(wmap_laea)
countries_laea_df<-fortify(countries_laea)

xmin<-min(capc_laea_df$lon)
xmax<-max(capc_laea_df$lon)
ymin<-min(capc_laea_df$lat)
ymax<-max(capc_laea_df$lat)
buff<-100000
...
ggplot() + 
  ...
  geom_point(data=capc_laea_df, aes(lon, lat), color="red") +
