...
```{r}
capg <- capg[capg$lon > 0 & capg$lon<25 & capg$lat > 43 , ]  #you will need to change these values to ones are relevant to your species.
...
Optional extras to make more attractive map using ggplot2 - you need to download the following data for this:
...
coordinates(capc)<-c("lat","lon")
proj4string(capc) <- CRS("+proj=longlat")
capc_df<-data.frame(capc)
wmap_df<-fortify(wmap)
countries_df<-fortify(countries)


xmin<-min(capc_df$lon)
xmax<-max(capc_df$lon)
ymin<-min(capc_df$lat)
ymax<-max(capc_df$lat)
buff<-0.5

ggplot() + 
  geom_polygon(data=wmap_df, aes(long,lat,group=group), fill="white")+
  geom_path(data=countries_df, aes(long,lat, group=group), color="light grey",size=0.1)+
  geom_point(data=capc_df, aes(lon, lat), color="red") +
